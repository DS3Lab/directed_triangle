...
class NodeIterPlusUndirected(override val uid: String) extends Transformer
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val degrees = neighbors.map { case (v, neighbors) => (v, neighbors.length) }
...
//      }.persist($(storageLevel))
    ...
    val edgeCandidates = neighbors.flatMap { case (v, nb) =>
        val candidates = NodeIterPlusUndirected.candidateEdges(v, nb, bcDegrees.value)
...
//    }.partitionBy(new HashPartitioner(partNum)).map(_._2)
    println(s"number of candidate edges = ${edgeCandidates.count()}, number of partitions = ${edgeCandidates.getNumPartitions}")
...
object NodeIterPlusUndirected {

  def candidateEdges(vid: VertexId,
                     neighbors: Array[VertexId],
                     degrees: Map[VertexId, Int]): Array[(VertexId, VertexId)] = {
    ...
    for (srcId <- neighbors; dstId <- neighbors) {
      if (srcId < dstId) {
        val srcDegree = degrees.getOrElse(srcId, 0)
        val dstDegree = degrees.getOrElse(dstId, 0)
